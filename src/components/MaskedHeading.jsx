...
const MaskedHeading = ({
  ...
  const place = useCallback(() => {
    ...
    const maxX = Math.max(0, ((s.fillScale - 1) / 2) * width);
    const maxY = Math.max(0, ((s.fillScale - 1) / 2) * height);
  ...
  const sync = useCallback(() => {
    ...
    root.style.fontSize = `${clamp(width * s.textScale, 20, 200).toFixed(1)}px`;
